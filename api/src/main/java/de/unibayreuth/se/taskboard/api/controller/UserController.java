...
public class UserController {
    ...
    // Add GET /api/users endpoint to retrieve all users.
    @Operation(
        ...
        responses = {
            @ApiResponse(
                ...
                description = "All users to a JSON array."
            )
        }
    )
    @GetMapping
    public ResponseEntity<List<UserDto>> getAll() {
        return ResponseEntity.ok(
            userService.getAll().stream().map(userDtoMapper::fromBusiness).toList()       
        );
    }

    // Add GET /api/users/{id} endpoint to retrieve a user by ID.
    @Operation(
    ...
    // Add POST /api/users endpoint to create a new user based on a provided user DTO.
    @Operation(
        ...
        responses = {
            ...
            @ApiResponse(
                responseCode = "400",
                description = "User with the provided ID or name already present."
            )
        }
    )
    @PostMapping
